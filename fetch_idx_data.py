...
def monitor_futunn_pages():
    ...
    try:
        ...
        # Sanitize: SH/SZ/CY/HK browser sparklines are corrupted (gu.qq.com canvas)
        # Clear spark_path to prevent sawtooth — use Tencent API sparklines instead
        for _bad_key in ['SH', 'SZ', 'CY', 'HK']:
            if _bad_key in indices_data:
                indices_data[_bad_key].pop('spark_path', None)
        if len(indices_data) >= 1:
            print('  Browser snapshot: ' + str(len(indices_data)) + ' indices (SH/SZ/CY/HK spark_path cleared)', file=sys.stderr)
            return indices_data
    except Exception as e:
...
def fetch_all_indices():
    ...
    # E. Merge browser snapshot spark_path for indices where canvas extraction works
    #     SH/SZ/CY/HK: browser canvas from gu.qq.com produces corrupted sawtooth sparklines,
    #     so only use browser spark_path for SPX/NDX/DJI/VIX (futunn canvas, clean extraction)
    #     Also fill price/change from browser if API data is missing
    _browser_spark_ok = {'SPX', 'NDX', 'DJI', 'VIX'}  # indices with reliable canvas extraction
    if browser_data:
        for bk in ['SPX', 'NDX', 'DJI', 'VIX', 'SH', 'SZ', 'CY', 'HK']:
            ...
            # Apply spark_path only for indices with reliable canvas extraction
            if bk in _browser_spark_ok and bd.get('spark_path'):
...
def assemble_output(index_data, date_str=None):
    ...
    for key, name, href, is_vix in idx_config:
        ...
        if entry:
            ...
            # Generate sparkline — prefer API minute data over canvas extraction
            # API data (minute_prices) = exact price values, never wrong
            # Canvas (spark_path) = fallback for VIX which lacks API minute data
...
def main():
    ...
    if args.use_archive:
        ...
        if archive:
            ...
            output = archive
            # Merge latest browser snapshot data into archive
            # SH/SZ/CY/HK: browser canvas from gu.qq.com produces corrupted sawtooth sparklines,
            # so only merge spark_path for SPX/NDX/DJI/VIX — for A-share/HK keep archive sparklines
            _browser_spark_ok = {'SPX', 'NDX', 'DJI', 'VIX'}
            _snap_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'idx_data', 'browser_snapshot.json')
            if os.path.exists(_snap_path):
                try:
                    with open(_snap_path) as _sf:
                        _snap = json.load(_sf)
                    for _k, _v in _snap.get('indices', {}).items():
                        if not _v.get('spark_path'):
                            continue
                        if 'indices' not in output:
                            output['indices'] = {}
                        if _k not in output['indices']:
                            output['indices'][_k] = {}
                        # Override price/change/chg_pct from browser snapshot (more current)
                        if _v.get('price'):
                            output['indices'][_k]['price'] = _v['price']
                        if _v.get('change') is not None:
                            output['indices'][_k]['change'] = _v['change']
                        if _v.get('chg_pct') is not None:
                            output['indices'][_k]['chg_pct'] = _v['chg_pct']
                        if _v.get('source'):
                            output['indices'][_k]['source'] = _v['source']
                        # Inject spark_path only for indices with reliable canvas extraction
                        if _k in _browser_spark_ok:
                            output['indices'][_k]['spark_path'] = _v['spark_path']
                            if 'sparklines' not in output:
                                output['sparklines'] = {}
                            output['sparklines'][_k] = _v['spark_path']
                        if _v.get('high'):
                            output['indices'][_k]['high'] = _v['high']
                        if _v.get('low'):
                            output['indices'][_k]['low'] = _v['low']
                        if _v.get('prev_close'):
                            output['indices'][_k]['prev_close'] = _v['prev_close']
                    print(f"  Merged browser data ({_browser_spark_ok}) spark_path from snapshot", file=sys.stderr)
                except Exception as _e:
                    print(f"  Spark merge skipped: {_e}", file=sys.stderr)
        else:
            print(f"  No archive found, fetching...", file=sys.stderr)
            data = fetch_all_indices()
            output = assemble_output(data, date_str)
